...
func TestSetDomain(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "domain set with different IP",
			record: &LocalDNSRecord{
				Name:  "test.local",
				Type:  LocalDNSTypeA,
				Value: "10.0.0.1",
		...
		{
			name: "domain set with different domain name",
			record: &LocalDNSRecord{
				Name:  "example.com",
				Type:  LocalDNSTypeA,
				Value: "192.168.1.1",
		...
		},
		{
			...
			setup: func() (PiholeAPI, *mockHTTPDoer) {
				mockClient := &mockHTTPDoer{
					DoFunc: func(req *http.Request) (*http.Response, error) {
						// Only domain set should be called, not authentication
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "SetDomain not implemented for type CNAME",
		},
		{
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "unable to set domain",
		},
		{
			name: "unexpected status code (not 200)",
			record: &LocalDNSRecord{
				Name:  "test.local",
				Type:  LocalDNSTypeA,
				Value: "192.168.1.1",
			...
			setup: func() (PiholeAPI, *mockHTTPDoer) {
				...
				mockClient := &mockHTTPDoer{
					DoFunc: func(req *http.Request) (*http.Response, error) {
						...
						// Second call: domain set with new session
						return createMockResponse(http.StatusOK, "{}"), nil
					},
				}
				api := NewPiholeAPI(&Config{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.verifyReq != nil && mockClient.DoFunc != nil {
				...
				mockClient.DoFunc = func(req *http.Request) (*http.Response, error) {
					// Only capture PUT requests (not auth requests)
					if req.Method == "PUT" {
			...
			err := api.SetDomain(ctx, tt.record)

			if (err != nil) != tt.wantErr {
				t.Errorf("setDomain() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			if tt.wantErr && tt.errMsg != "" {
				if err == nil || !strings.Contains(err.Error(), tt.errMsg) {
					t.Errorf("setDomain() error = %v, want error containing %v", err, tt.errMsg)
				}
				return
			}
